...
export class AuditLogsInterceptor implements NestInterceptor {
  ...
  constructor(@Inject(SERVICES.SYSTEM) private systemClient: ClientRMQ) {}
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    return next.handle().pipe(
      map(async (res) => {
        ...
        if (Object.values(AuditLogEventList).includes(eventId as any)) {
          const events = [];
          ...
          this.systemClient.emit(ADMIN_CREATE_LOG, data);
        }
        return res;
      })
    );
